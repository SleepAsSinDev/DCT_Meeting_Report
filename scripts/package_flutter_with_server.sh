...
case "$target" in
  linux)
    ...
    dest_dir="$repo_root/flutter_app/build/linux/x64/release/bundle"
    dest_name="meeting_server"
    ;;
  windows)
    bin_src="$server_bin_windows"
    dest_dir="$repo_root/flutter_app/build/windows/x64/runner/Release"
  ...
  macos)
    ...
    app_path=$(ls -d "$repo_root"/flutter_app/build/macos/Build/Products/Release/*.app 2>/dev/null | head -n1 || true)
    if [[ -z "$app_path" ]]; then
      app_path="$repo_root/flutter_app/build/macos/Build/Products/Release/meeting_minutes_app.app"
    fi
